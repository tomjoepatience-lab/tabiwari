...
- `GEMINI_API_KEY` … レシートOCR用（唯一のプロバイダ。未設定なら 500）。https://aistudio.google.com/apikey で取得（無料枠）。OCRは1ユーザー1日5回まで（`user_settings.last_ocr_on`/`ocr_used`・JST暦日でサーバー側が永続制限）
- `ANTHROPIC_API_KEY` … 現在未使用（レシートOCRのClaudeフォールバックは撤去済み。`@anthropic-ai/sdk` は依存として残置・将来利用の余地）
- `PORT` … 既定 3000
