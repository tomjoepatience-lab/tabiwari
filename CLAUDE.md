...
**ユーザー認証（ユーザー名＋パスワード＋cookieセッション）＋家族グループ**で、ログインした人は自分の所属グループのプロジェクトだけ見える＝共有家計簿。トップは**ダッシュボード**で、`trips` を一般化した「プロジェクト（`kind`: `trip`=旅行 / `daily`=日常、`group_id`でグループに属する）」をカード表示し、押すと各プロジェクトに入る。全プロジェクト横断の支出を**統括**として可視化。
レシート（割り勘の証拠）とは別に「思い出写真」を旅行へアップロードでき、それだけを時系列に組版した**スクラップブック風アルバム**（tabikake 由来）を表紙＋ページめくりで閲覧できる。拡張枠として OCR(M3)・AI相談を設計に残す。
